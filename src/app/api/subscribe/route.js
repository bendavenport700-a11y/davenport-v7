...
export async function POST(req) {
  ...
  try {
    ...
    // One recurring subscription line item per inventory item
    const lineItems = items.map(item => ({
      price_data: {
        ...
        unit_amount: item.rent_price, // stored in cents
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // Save card for future off-session use (deposit authorization after checkout)
    ...
    // Insert a pending order row per item
    for (const item of items) {
      await sql`
        INSERT INTO orders (clerk_id, stripe_session_id, stripe_customer_id, inventory_id, amount, status)
        VALUES (${userId}, ${session.id}, ${customerId}, ${item.id}, ${item.rent_price}, 'pending')
      `;
    }
